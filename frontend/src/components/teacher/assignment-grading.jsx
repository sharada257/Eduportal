...
import { CheckCircle, Clock, Eye, MessageSquare, Download, Upload, Filter, Search, AlertCircle } from "lucide-react"


export default function AssignmentGrading({ assignments, setAssignments, courses }) {
          ...
          return {
            ...
            submissions: assignment.submissions.map((submission) => {
              if (submission.studentId === studentId) {
                return { ...submission, grade, feedback, gradedAt: new Date().toISOString() }
              }
              return submission
            }),
          }
              ...
              <Card key={assignment.id} className="hover:shadow-lg transition-shadow cursor-pointer">
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    >
                      {stats.pending > 0 ? `Grade ${stats.pending} Submissions` : "All Graded"}
                    </Button>
                  </div>
                </CardContent>
              </Card>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-xl font-semibold mt-2">{selectedAssignment.title}</h3>
              <p className="text-gray-600">
                {selectedAssignment.course} • Due: {selectedAssignment.dueDate}
              </p>
            </div>
            <Badge variant="outline">
              {getSubmissionStats(selectedAssignment).graded} / {getSubmissionStats(selectedAssignment).total} Graded
            </Badge>
          </div>
          ...
          {gradingSubmission ? (
              ...
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold">{gradingSubmission.studentName}</div>
                      <div className="text-sm text-gray-600">Submitted: {gradingSubmission.submittedAt}</div>
                    </div>
                  </div>
                ...
                <Tabs defaultValue="submission" className="space-y-6">
                                ...
                                {[
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      ...
                                      if (input) input.value = grade.value.toString()
                                    }}
                                  >
                                    {grade.label}
                                  </Button>
                            ...
                            <Button
                              onClick={() => {
                                ...
                                if (grade >= 0 && grade <= 100) {
                                  gradeAssignment(selectedAssignment.id, gradingSubmission.studentId, grade, feedback)
                                }
                              }}
                  ...
                  <TabsContent value="rubric">
                    <Card>
                      ...
                      <CardContent>
                        <p className="text-gray-500">Rubric-based grading coming soon...</p>
                      </CardContent>
                    </Card>
                  </TabsContent>
                </Tabs>
            ...
            <div className="space-y-4">
              {selectedAssignment.submissions.map((submission) => (
                <Card key={submission.studentId} className="hover:shadow-md transition-shadow">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-600">Submitted: {submission.submittedAt}</div>
                        </div>
                      </div>
